...
public class PostServiceImplTest {
    ...
    @Test
    public void should_createNewParentPost_when_valid_RequestDTO() {
        Post post1 = buildPost1();
        PostDTO postDTO1 = buildPost1DTO();
        ...
        given(userService.findById(principalID)).willReturn(Optional.of(post1.getUser()));
        given(postRepository.save(post1)).willReturn(post1);
        given(principalService.getPrincipalUserId()).willReturn(Optional.of(1L));
        given(postMapper.postToPostDto(post1)).willReturn(postDTO1);
        ...
        then(userService).should().findById(principalID);
        ...
        assertThat(postDTO.getUser().getId(), is(principalID));
    ...
    public void should_throwBadRequestException_when_postRequestDtoContainsCategoryIdThatDoesNotExist() {
        ...
        assertThrows(BadRequestException.class, () -> postService.createNewPost(requestDTO));
    }
    ...
    public void should_saveParentAndChild_AndSetURLs_when_createNewChildPost() {
        ...
        given(principalService.getPrincipalUserId()).willReturn(Optional.of(1L));
        given(userService.findById(1L)).willReturn(Optional.of(child1.getUser()));
    ...
    public void should_throwException_when_parentPostId_doesNotExist() {
        ...
        assertThrows(BadRequestException.class, () -> postService.createNewChildPost(parentID, request1DTO));
    }
...
//    }
    ...
    private Post buildPost1() {
        Category cat1 = DomainBuilder.buildCategory(CAT1_ID, CAT1_NAME);
        User user1 = DomainBuilder.buildUser(USER_ID, USER_NAME, "first", "last", "email@example.com", "password", "secret");
        Post post1 = DomainBuilder.buildPost(POST1_ID, POST1_TITLE, POST1_TEXT, POST1_IMAGE_URL, cat1, user1, null);
        return post1;
    }
    ...
    private Post buildChild1() {
        Category cat1 = DomainBuilder.buildCategory(CAT1_ID, CAT1_NAME);
        User user1 = DomainBuilder.buildUser(USER_ID, USER_NAME, null, null, null, null, null);
        Post post1 = DomainBuilder.buildPost(POST1_ID, POST1_TITLE, POST1_TEXT, null, cat1, user1, null);
        Post child1 = DomainBuilder.buildPost(CHILD1_ID, CHILD1_TITLE, CHILD1_TEXT, null, cat1, user1, post1);
    ...
    private PostDTO buildPost1DTO() {
        ...
        return DomainBuilder.buildPostDTO(POST1_ID, postUserDTO, POST1_TITLE, POST1_TEXT, POST1_IMAGE_URL, categoryDTO, LocalDateTime.now(), new ArrayList<>());
    }

    private PostDTO buildChild1DTO() {
        PostUserDTO postUserDTO = PostUserDTO.builder().id(USER_ID).userName(USER_NAME).userUrl(USER_URL).build();
        CategoryDTO categoryDTO = CategoryDTO.builder().id(CAT1_ID).name(CAT1_NAME).categoryUrl(CAT1_URL).build();
        return DomainBuilder.buildPostDTO(CHILD1_ID, postUserDTO, CHILD1_TITLE, CHILD1_TEXT, POST1_IMAGE_URL, categoryDTO, LocalDateTime.now(), new ArrayList<>());
    }
}